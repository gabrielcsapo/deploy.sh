...
module.exports = {
    ...
    methods: {
        redeploy: function() {
            ...
            Utils.get('/api/process/' + this.name + '/redeploy', function(response) {
                if (response.success) {
                    self.deployed = true;
        ...
        editSave: function() {
            ...
            Utils.post('/api/process/' + self.name + '/settings', self.config, function(status, response) {
                if(status === 200) {
    ...
    created: function() {
        var self = this;
        var application = new RegExp('^/application/((?:[^/]+?))(?:/(?=$))?$', 'i').exec(window.location.pathname)[1];
        if (typeof window !== 'undefined') {
            ...
            Utils.get('/api/process/' + application + '/json', function(response) {
