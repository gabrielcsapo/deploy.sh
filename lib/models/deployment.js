...
class Deployment {
  ...
  static async proxy(req, res) {
    ...
    if(!deployment) {
      return res.status(404).sendFile(new URL('../static/not-found.html', import.meta.url));
    }
    ...
    proxy.on('error', () => {
      res.status(502).sendFile(new URL('../static/page-could-not-load.html', import.meta.url));
    });
