...
export default async function deploy({ name, bundlePath, token, username }) {
  const deployment = await Deployment.get({ username, token, name, create: true });
  ...
  if (config.type === 'static') {
    await writeFile(path.resolve(outputDir, 'index.js'), (await readFile(new URL('static', 'static-server.js', import.meta.url))).toString('utf8'));
  }
