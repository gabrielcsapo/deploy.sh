...
app.post('/upload', User.authenticateMiddleware, asyncMiddleware(async (req, res) => {
  try {
    const { user } = req;
...
app.get('/api/deployments/:name/logs', User.authenticateMiddleware, asyncMiddleware(async (req, res) => {
...
app.get('/api/deployments', User.authenticateMiddleware, asyncMiddleware(async (req, res) => {
  const { token, username } = req.user;
...
app.delete('/api/deployments/:name', User.authenticateMiddleware, asyncMiddleware(async (req, res) => {
  const { name } = req.params;
  const { token, username } = req.user;
...
app.get('/api/deployments/:name', User.authenticateMiddleware, asyncMiddleware(async (req, res) => {
  const { name } = req.params;
  const { token, username } = req.user;
...
app.get('/api/logout', User.authenticateMiddleware, asyncMiddleware(async (req, res) => {
  const { token, username } = req.user;
